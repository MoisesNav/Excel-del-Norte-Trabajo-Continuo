...
def procesar_catalogo_exel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    columnas_eliminar = [
        ...
        'familia_nombre', 'subcategoria_nombre', 'descripcion_extendida', 
...
def cargar_temporal(df: pd.DataFrame):
    """Carga masiva ultra rápida usando COPY FROM."""
    engine = get_db_engine()
    if not engine: return
    
    try:
        # Convertimos el DataFrame a un "archivo" en memoria (CSV sin cabecera)
        buffer = io.StringIO()
        df.to_csv(buffer, index=False, header=False, sep='\t', na_rep='NULL')
        buffer.seek(0)
        
        # Obtenemos el nombre de las columnas para el comando COPY
        ...
        with engine.begin() as conn:
            ...
            # 2. Uso del cursor raw de psycopg2 para COPY
            raw_conn = conn.connection
            cursor = raw_conn.cursor()
            cursor.copy_from(buffer, 'temp_tbl_exel', sep='\t', null='NULL', columns=df.columns)
            
            logger.info(f"¡Éxito! {len(df)} registros cargados en pocos segundos.")
